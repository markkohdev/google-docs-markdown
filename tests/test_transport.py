"""
Tests for Google Docs Transport.

Tests authentication, document retrieval, and error handling for the raw transport layer.
"""
...
from google_docs_markdown.transport import (
    SCOPES,
    GoogleDocsTransport,
)
...
class TestExtractDocumentID:
    ...
    def test_extract_from_edit_url(self) -> None:
        ...
        assert GoogleDocsTransport.extract_document_id(url) == "abc123def456"
    ...
    def test_extract_already_extracted_id(self) -> None:
        ...
        doc_id = "abc123def456"
        assert GoogleDocsTransport.extract_document_id(doc_id) == doc_id
    ...
    def test_extract_invalid_url(self) -> None:
        ...
        with pytest.raises(ValueError, match="Could not extract document ID"):
            GoogleDocsTransport.extract_document_id("invalid")
...
class TestAuthentication:
    ...
    def test_custom_credentials(self) -> None:
        ...
        mock_creds = Mock()
        client = GoogleDocsTransport(credentials=mock_creds)
...
class TestGetDocument:
    ...
    def test_get_document_success(self, mock_build: Any) -> None:
        ...
        client = GoogleDocsTransport(credentials=Mock())
    ...
    def test_get_document_real_response_structure_validation(self, mock_build: Any) -> None:
        """Validate that the transport correctly handles complex real document structures."""
        example_doc = load_example_document_json()
        mock_service = Mock()
        mock_request = Mock()
        mock_request.execute.return_value = example_doc
